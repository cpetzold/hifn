...
test("intoObject", t => {
  ...
  t.deepEqual(intoObject([["foo", "bar"], ["hello", "world"]]), {
    foo: "bar",
    hello: "world"
  });
  t.deepEqual(intoObject([["foo", "bar"], ["number", 1]] as [string, any][]), {
    foo: "bar",
    number: 1
  });
});
...
test("map", t => {
  ...
  t.deepEqual(map((x: number) => x + 1, [1, 2, 3]), [2, 3, 4]);
...
test("thread", t => {
  t.deepEqual(thread([1, 2, 3], map((x: number) => x + 1)), [2, 3, 4]);
});
